...
2. line88， **build_anchor_generator**()方法创建了实例。
   ...
   **gen_single_level_base_anchors**()方法先对单个位置 (0,0) 生成 base anchors。ratios是高宽比(h/2)，0.5即h是w的一半。其余参数见下方代码注释。
...
4. line 158，self.anchor_generator.**grid_anchors()**方法得到5个特征图上的anchors。
   ...
   **grid_anchors()**方法对每张特征图逐一处理，调用anchors = self.**single_level_grid_anchors**()方法。
   ...
   **single_level_gride_anchors()**方法利用输入特征图尺寸加上 base anchors，得到每个特征图位置的对于原图的 anchors。
...
5. line 165, **valid_flags()**与pad_shape有关。在mmdetection/mmdet/datasets/pipelines/formating.py文件line273中解释， "pad_shape": image shape after padding。这通常pad_shape就是img_shape。
