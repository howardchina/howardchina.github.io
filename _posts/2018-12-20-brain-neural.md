...
K. J. Frison et al. MRC Cyclotron Unit, Hammersmith Hospital, London, U.K.

摘要：关于功能连接的解释，定义为神经生理学的指数的时序关联。测得变化主要有两部分的原因。第一，突出强调有意识的大脑系统中一块和语言流利有关的地方；第二，anterior cingulate和Wernicke's area 反映单调的时间影响。这个大脑系统有注意力偏差。

通过 extrinsic cortical connections 描述 brain function (Goldman Rakic, 1998)。采用PET （positron emission tomography）对全脑状态均匀采样，缺点是时空分辨率差。

**方法：**

**递归PCA分析**

对数据矩阵M不断地PCA分解，提取最大的特征向量。

```matlab
function [e,v] = theta(M)
% recursive PCA analysis
% format [e,v] = theta(M)
% M = data matrix with 2 n columns and O rows where O < k = 16
% e-eigenvectors
% v-eigenvalues
[x,y] = size(M)
k = 16
...
s = 2*k
if y <= s
   [e,v] = eig( conv(M) ); [v,i] = sort( diag(v) ); e(:,i) = e;
   e = e(:,[(1 + k):s]); v = v([(1+k):s]);
   return
end
y = y/2;
x = [1:y]; y = [1:y] + y
e1 = theta( M(:,x) ); e2 = theta( M(:,y) );
[e,v] = theta( [M(:,x)*e1 M(:,y)*e2 ] );
e = [ e1 zeros(e1); e2 zeros(e2)]*e;
```
...
page10，提到两种Functional connectivity 的技术

1. coherence. 
   $$
   fc_{ij}(x)=\sigma_{ij}(w)=|g_{ij}(w)^2|/g_{ii}(w)\cdot g_{jj}(w)
   $$
   gij(w): cross-spectral density; gii and gjj: autospectral densities。一种分析随机时序信号的方法。
...
大致理解，（对于脑中的两个区域，）两者从数据分布关联上定义Functional Connectivity。

---

## Implementation of a new parcellation of the orbitofrontal cortex in the automated anatomical labeling atlas [NeuraoImage 2015]

Edmund T. Rolls et al.

Oxford Centre for Computational Neuroscience, Sxford, UK

**摘要：** 名为AAL2的工具，用于parcellation of orbitofrontal cortex。[AAL2 for SPM12.tar.gz](http://www.gin.cnrs.fr/en/tools/aal-aal2/)